...
async function getAddress(req, res) {
  try {
    ...
    // const rsaDCipher =  .createDecipher(rsaAlg, rsaKeys.privateKey);
    ...
    const sig = sign.once(data)
    let verify = session.createVerify(alg, keys.publicKey)
    const ver = verify.once(data, sig)
...
const lib = path.join(__dirname, `../../../lib/eps2003csp11.dll`)
...
try {
  ...
  session.login('11112222', graphene.UserType.USER)
  session.setPin('12345678', 'new pin')
  session.logout()
  session.close()
